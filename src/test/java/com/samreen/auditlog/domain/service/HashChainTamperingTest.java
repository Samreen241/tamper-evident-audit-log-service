...
class HashChainTamperingTest {
  ...
  void detectsChangedContent() {
    ...
    var valid =
        new AuditRecord(draft.id(), 1, event, draft.previousHash(), hashing.contentHash(draft));
    var changed =
        new AuditRecord(
            valid.id(),
            1,
            new AuditEvent("LOGIN", "actor", "USER", "id", Map.of("ok", false), event.timestamp()),
            valid.previousHash(),
            valid.contentHash());
    var result = hashing.verify(List.of(changed));
    assertThat(result.intact()).isFalse();
    assertThat(result.violationType())
        .isEqualTo(VerificationResult.ViolationType.CONTENT_HASH_MISMATCH);
  }
}
