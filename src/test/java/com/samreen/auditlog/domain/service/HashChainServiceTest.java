...
class HashChainServiceTest {
  ...
  @BeforeEach
  void setUp() {
    service = new HashChainService(new CanonicalJsonService(new ObjectMapper()));
    timestamp = Instant.parse("2026-08-13T10:15:30Z");
  }

  @Test
  void sameInputProducesSameHash() {
    AuditRecord record =
        record(1, event(Map.of("b", 2, "a", 1)), HashChainService.GENESIS_HASH, "");
  ...
  void differentPayloadProducesDifferentHash() {
    ...
    AuditRecord second =
        record(1, event(Map.of("value", "two")), HashChainService.GENESIS_HASH, "");
  ...
  void firstRecordUsesGenesisAndSecondReferencesFirst() {
    AuditRecord firstWithoutHash =
        record(1, event(Map.of("value", 1)), HashChainService.GENESIS_HASH, "");
    AuditRecord first =
        record(
            1,
            firstWithoutHash.event(),
            HashChainService.GENESIS_HASH,
            service.contentHash(firstWithoutHash));
    AuditRecord secondWithoutHash = record(2, event(Map.of("value", 2)), first.contentHash(), "");
    AuditRecord second =
        record(
            2,
            secondWithoutHash.event(),
            first.contentHash(),
            service.contentHash(secondWithoutHash));
  ...
  void timestampIsNormalizedAsInstant() {
    AuditEvent first =
        new AuditEvent(
            "LOGIN", "actor", "USER", "id", Map.of(), Instant.parse("2026-08-13T10:15:30Z"));
    AuditEvent second =
        new AuditEvent(
            "LOGIN", "actor", "USER", "id", Map.of(), Instant.parse("2026-08-13T12:15:30+02:00"));
  ...
  private AuditEvent event(Map<String, Object> payload) {
    return new AuditEvent("LOGIN", "actor", "USER", "id", payload, timestamp);
  }

  private AuditRecord record(long sequence, AuditEvent event, String previous, String hash) {
    return new AuditRecord(UUID.randomUUID(), sequence, event, previous, hash);
  }
}
