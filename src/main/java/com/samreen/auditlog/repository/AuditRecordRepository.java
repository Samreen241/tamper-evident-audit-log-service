...
public interface AuditRecordRepository
  ...
  @Modifying
  @Query(
      "update AuditRecordEntity record set record.status = 'ARCHIVED' where record.status = 'ACTIVE' and record.eventTimestamp < :cutoff")
  ...
  Page<AuditRecordEntity> findByResourceTypeAndResourceId(
