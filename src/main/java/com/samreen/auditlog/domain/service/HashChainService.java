...
public class HashChainService {
  ...
  public HashChainService(CanonicalJsonService canonicalJsonService) {
    this.canonicalJsonService = canonicalJsonService;
  }

  public String contentHash(AuditRecord record) {
    return sha256(canonicalJsonService.canonicalize(record.hashContent()));
  }
    ...
    for (int index = 0; index < records.size(); index++) {
      ...
      if (record.sequenceNumber() != sequence)
        return invalid(
            records,
            index,
            VerificationResult.ViolationType.SEQUENCE_GAP,
            "Unexpected sequence number");
      if (!previous.equals(record.previousHash()))
        return invalid(
            records,
            index,
            VerificationResult.ViolationType.PREVIOUS_HASH_MISMATCH,
            "Previous hash mismatch");
      if (!"REDACTED".equals(record.status()) && !contentHash(record).equals(record.contentHash()))
        return invalid(
            records,
            index,
            VerificationResult.ViolationType.CONTENT_HASH_MISMATCH,
  ...
  private VerificationResult invalid(
      List<AuditRecord> records, int index, VerificationResult.ViolationType type, String message) {
